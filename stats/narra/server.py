...
PROPERTY_ENUMS = {
    ...
    "source": {"reader", "character", "chat", "scene"},
...
SESSION_EVENTS = {
    "reading_session_qualified", "ai_request_started", "media_job_enqueued",
...
FEATURE_BY_EVENT = {
    ...
    "book_analysis_started": "Character analysis",
...
PRODUCT_ACTION_EVENTS = {
    ...
    "book_analysis_started", "media_job_enqueued", "tts_playback_started",
}
VALUE_ACTION_EVENTS = {
    "book_import_completed", "reading_session_qualified", "chapter_completed",
    "bookmark_added", "note_added", "ai_request_completed", "book_analysis_completed",
    "media_job_completed",
}
...
def _period_starts(now: float, days: float) -> tuple[float, dict[str, float]]:
    ...
    return float(selected_days), {
        ...
        # Fleet contract since 01.08.2026: MAU is the rolling last 30
        # Moscow dates, avoiding the artificial first-of-month cliff.
        "mau": (day_start - timedelta(days=29)).timestamp(),
    }


def _user_value_events(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Successful, user-origin Narra outcomes; retries never enter here."""
    starts = {
    ...
    for row in rows:
        ...
        if (
            row["name"] != "ai_request_completed"
            or row["properties"].get("origin") == "background"
            or not request_id
...
def _product_kpis(rows: list[dict[str, Any]], now: float, wau_start: float) -> dict[str, Any]:
    value_events = _user_value_events(rows)
    value_wau = len({
        row["device_id"] for row in value_events if wau_start <= row["ts"] <= now
    })

    first_seen: dict[str, float] = {}
    for row in rows:
        if row["name"] != "app_opened":
            continue
        first_seen[row["device_id"]] = min(
            first_seen.get(row["device_id"], row["ts"]), row["ts"]
        )
    values_by_actor: dict[str, list[float]] = defaultdict(list)
    for row in value_events:
        values_by_actor[row["device_id"]].append(row["ts"])

    activation_eligible = {
        actor: started_at for actor, started_at in first_seen.items()
        if started_at <= now - 7 * 86400
    }
    activated = {
        actor for actor, started_at in activation_eligible.items()
        if any(started_at <= timestamp < started_at + 7 * 86400
               for timestamp in values_by_actor.get(actor, []))
    }

    first_value = {
        actor: min(timestamps) for actor, timestamps in values_by_actor.items()
        if timestamps
    }
    retention_eligible = {
        actor: started_at for actor, started_at in first_value.items()
        if started_at <= now - 7 * 86400
    }
    retained = {
        actor for actor, started_at in retention_eligible.items()
        if any(timestamp >= started_at + 7 * 86400
               for timestamp in values_by_actor.get(actor, []))
    }
    return {
        "value_wau": value_wau,
        "activation_7d": {
            "rate": _percent(len(activated), len(activation_eligible))
            if activation_eligible else None,
            "activated": len(activated),
            "eligible": len(activation_eligible),
        },
        "value_retention_d7": {
            "rate": _percent(len(retained), len(retention_eligible))
            if retention_eligible else None,
            "retained": len(retained),
            "eligible": len(retention_eligible),
        },
    }
...
def _is_background_event(row: dict[str, Any]) -> bool:
    return row["properties"].get("origin") == "background"
...
def _feature_for_event(row: dict[str, Any]) -> str | None:
    if row["name"] == "ai_request_started":
        return {
    ...
    if row["name"].startswith("media_job_"):
        return {
            ...
            "chapter_markup": "Chapter analysis",
        }.get(str(row["properties"].get("job_type")), "Media")
    if row["name"].startswith("tts_"):
        return "Audio playback"
    return FEATURE_BY_EVENT.get(row["name"]) or row["properties"].get("feature")
...
def compute_dashboard(days: float = 1.0) -> dict[str, Any]:
    ...
    purpose_request_ids: dict[str, set[tuple[str, str]]] = defaultdict(set)
    purpose_users: dict[str, set[str]] = defaultdict(set)
    for row in request_rows:
        ...
        purpose_request_ids[purpose].add(key)
        purpose_users[purpose].add(row["device_id"])
    ...
    for row in active:
        feature = _feature_for_event(row)
        if feature:
    ...
    product_actions = sum(
        row["name"] in PRODUCT_ACTION_EVENTS and not _is_background_event(row)
        for row in selected
    )
    value_actions = sum(
        row["name"] in VALUE_ACTION_EVENTS and not _is_background_event(row)
        for row in selected
    )
    feature_breadth = len({
        (
            ...
            _feature_for_event(row),
        )
        for row in active
        if _feature_for_event(row)
    })
    ...
    purpose_metrics = []
    for purpose, ids in sorted(
        purpose_request_ids.items(), key=lambda item: (-len(item[1]), item[0])
    ):
        eligible = ids & outcome_eligible_ids
        purpose_metrics.append({
            "name": purpose,
            "requests": len(ids),
            "actors": len(purpose_users[purpose]),
            "completed": len(ids & completed_ids),
            "failed": len(ids & failed_only_ids),
            "outcome_eligible": len(eligible),
            "success_rate": _percent(len(ids & completed_ids & eligible), len(eligible))
            if eligible else None,
        })
    product_kpis = _product_kpis(rows, now, period_starts["wau"])
    metric_cards = [
    ...
    metric_cards.append({
        "id": "product.narra.value_wau",
        "version": 1,
        "label": "Narra value WAU",
        "value": str(product_kpis["value_wau"]),
        "value_numeric": product_kpis["value_wau"],
        "value_type": "number",
        "unit": "actors",
        "comparability": "product",
        "hint": "Unique actors receiving a successful user-origin Narra result in the current Moscow ISO week.",
    })
    for identifier, label, key, hint in (
        (
            "product.narra.activation_7d", "Narra activation · 7d", "activation_7d",
            "Share of mature first app-open cohorts receiving a successful Narra result within seven days.",
        ),
        (
            "product.narra.value_retention_d7", "Narra value retention · D7+",
            "value_retention_d7",
            "Share of mature first-value cohorts receiving another successful Narra result at least seven days later.",
        ),
    ):
        value = product_kpis[key]["rate"]
...
@app.post("/delete")
async def delete_actor(request: Request) -> JSONResponse:
    """Server-to-server erasure for one HMAC-pseudonymous analytics actor."""
    if not INGEST_TOKEN:
        return _response({"error": "ingest disabled"}, 503)
    provided = request.headers.get("X-Ingest-Token", "").encode()
    if not hmac.compare_digest(provided, INGEST_TOKEN.encode()):
        return _response({"error": "unauthorized"}, 401)
    raw = await request.body()
    if not raw or len(raw) > 1024:
        return _response({"error": "invalid payload"}, 400)
    try:
        payload = json.loads(raw)
    except (json.JSONDecodeError, UnicodeDecodeError):
        return _response({"error": "invalid json"}, 400)
    if not isinstance(payload, dict) or set(payload) not in ({"actor_id"}, {"actorId"}):
        return _response({"error": "invalid payload"}, 400)
    actor_id = payload.get("actor_id", payload.get("actorId"))
    if not isinstance(actor_id, str) or not ACTOR_RE.fullmatch(actor_id):
        return _response({"error": "invalid actor"}, 400)
    with DB_LOCK:
